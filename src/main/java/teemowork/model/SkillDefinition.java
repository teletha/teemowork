...
public interface SkillDefinition {
    ...
    public static void Ashe(Champion champion, Skill P, Skill Q, Skill W, Skill E, Skill R) {
        ...
        Q.update(P604)
                ...
                .active("4スタックを消費して5秒間{1}し、通常攻撃が5回に分けて合計{2}を与えるようになる。On-Hit Effectsは5連射の初撃のみ有効(Hurricaneは5回発動)。")
    ...
    public static void Ryze(Champion champion, Skill P, Skill Q, Skill W, Skill E, Skill R) {
        P.update(P603)
                .passive("スキル使用時に6秒間、スタックを得る。5スタック時に、{1}間フルチャージ状態になり、{2}を得て、5回スキルを使用するたびに使用したスキル以外のスキルの{3}が「", Q, "のCD数値分」解消する。")
        ...
        Q.update(P604)
                ...
                .variable(1, MagicDamage, 60, 35, ap(0.55), amplify(Mana, 0.02, 0.005))
                .mana(40)
                .cd(4)
                .range(900);
    ...
    public static void Sivir(Champion champion, Skill P, Skill Q, Skill W, Skill E, Skill R) {
        ...
        W.update(P524)
                ...
                .variable(1, Radius, 450)
                .variable(2, PhysicalDamage, amplify(AD, 0.6, 0.05))
                .variable(3, ResetAATimer)
                .mana(60)
                .cd(12, -1.5);
    ...
    public static void Yorick(Champion champion, Skill P, Skill Q, Skill W, Skill E, Skill R) {
        ...
        Q.update(P618)
                .active("次に行う通常攻撃は追加{1}を与え{2}する。HPが50%未満の場合は回復量が倍になる。このスキルは使用時に{3}する。このスキルで敵を倒した場合は墓が作り出される。周囲に3つ以上の墓が存在し、このスキルがCD中であった場合再度このスキルを使用するとすべての墓を消滅させて最大4つまでの範囲内の墓の位置にミストウォーカーを召喚する。")
